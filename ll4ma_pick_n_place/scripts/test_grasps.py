...
linestyle = ['dotted','dotted', '--', 'dotted', '--'] + ['-'] * 50
colors = ['#ff0000', '#00ff00', '#00ff00', '#ffff00', '#ffff00'] + \
...
if __name__ == '__main__':
    ...
    grasp_cli = GraspUtils()
    grasp_cli.init_mdn(obj_grid.sparse_grid, obj_grid.grid_size, obj_grid.true_size, N)
    means = grasp_cli.means
    iters = np.linspace(-0.1, 0.1, N)
    grasp_confs = np.tile(means[:,:,1], (N,1))
    grasp_confs[:,1] += iters
    prior, wtcomps, comps = grasp_cli.test_comp_mixture(grasp_confs)
    pyplot.plot(range(N), prior, label="mixture")
    for i in range(comps.shape[1]):
        pyplot.plot(range(N), wtcomps[:,i], label="wt_comp_"+str(i+1))
        pyplot.plot(range(N), comps[:,i], label="comp_"+str(i+1))
    pyplot.legend()
    pyplot.show()

    pyplot.plot(range(N), np.exp(-prior), label="mixture")
    for i in range(comps.shape[1]):
        pyplot.plot(range(N), np.exp(-wtcomps[:,i]), label="wt_comp_"+str(i+1))
        pyplot.plot(range(N), np.exp(-comps[:,i]), label="comp_"+str(i+1))
    pyplot.legend()
    pyplot.show()
