...
def joint_init(pblm, num_inits=5, full_cost=False, debug=False):
    ...
    for ini in range(num_inits):
        ...
        pitches = np.array([0.])
        rolls = np.array([-0.8,-1.4, 0.0, 0.8, 1.4])
        yaws = np.array([0.0])
        zs = np.arange(place_heuristic.place_bounds[0][2], place_heuristic.place_bounds[1][2], 0.005)
        pperms = np.array(np.meshgrid(rolls, pitches, zs, yaws)).T.reshape(-1,4)
        
        free_pts = np.zeros((pperms.shape[0], 6))
        free_pts[:, :2] = XT
        free_pts[:, 2] = pperms[:, 2]
        free_pts[:, 3] = pperms[:, 0]
        free_pts[:, 4] = pperms[:, 1]
...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        ...
        place_heuristic = heurs.minimize_height(obj_grid, XT, Env_List)
        #exit(0)
        ...
        for ninit,init in enumerate(inits[:]):
            ...
            if executing:
                ...
                rospy.sleep(2)
                planner.remove_object(GRASP_OBJ_FN)
                planner.remove_object('grasp_object')
            ...
            pre_place_joints = pblm.get_lift_up_joints(solret.solution[pblm.P_IDX], dist=0.2)
            ...
            if executing:
                ...
                post_place_joints = pblm.get_approach_head(solret.solution[pblm.P_IDX])
                move_q_vel = post_place_joints - solret.solution[pblm.P_IDX]
                move_traj = planner.q_vel_traj(pblm.arm_palm_KDL, move_q_vel)
                planner.execute_plan(move_traj)
        ...
        if input('Add env?').lower() in ['y', 'yes']:
            obj_n = None
            e_grid = get_objgrid(True)
            e_grid.gen_sd_map()
            e_grid.Trans = pblm.ptable_world_mat @ e_grid.compute_pretrans()
            esize = e_grid.get_bounds_at_config()
            esize = esize[1] - esize[0]
            while (not obj_n) or (obj_n in Env_List): 
                obj_n = input('Input object identifier: ')
            Env_List[obj_n] = e_grid
            planner.add_object_bb(esize, obj_n, e_grid.get_pose().pose, 'place_table_corner')
