...
if __name__ == '__main__':
    rospy.init_node('pnp_moveit_test_node')
    ...
    input("Press to continue tests")

    plan_traj = planner.plan_to_joint(x_sol)
    planner.execute_plan(plan_traj)
    hand.grasp()

    input("Press to continue tests for cartesion plans")

    lift_traj = planner.lift_up()
    planner.execute_plan(lift_traj)

    input("Press to continue tests for second joint target")

    plan_traj = planner.plan_to_joint([-q for q in x_sol])
    planner.execute_plan(plan_traj)
    rospy.sleep(2)
    hand.release()
