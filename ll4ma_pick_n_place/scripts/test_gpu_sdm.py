...
from ll4ma_pick_n_place.planning_client import moveit_client, reflex_client, get_display_state, display_robot_state
...
#OBJ_NAME_ = ["006_mustard_bottle", "025_mug", "024_bowl", "054_softball"]
OBJ_NAME_ = ["003_cracker_box", "019_pitcher_base", "006_mustard_bottle", "054_softball", "025_mug", "024_bowl"]
...
if __name__ == '__main__':
    ...
    for i in range(place):
        ...
        x_sol = [ 2.75727465, -1.57946291, -1.4094462, 1.72657426, -2.879, -1.51238738, 2.879] 
        delt = 0.01
        ...
        #obj_grid.pose = place_conf.pose
        #PartialEnv.ObjList[obj_n] = obj_grid
        #PartialEnv.expand_grid()

        sdm3d = obj_grid.gen_sd_map()
        surf_pts = obj_grid.get_level_set(10)
        qst = time.time()
        obj_grid.query_points(surf_pts)
        print("points_queried in {} secs".format(time.time() - qst))
        delete_model_client(obj_n)
        exit(0)
        
        print("SDM size: ", round(sys.getsizeof(sdm3d)/1e+9,4))
        #sdm2d = min_from_sdm3d(sdm3d)
        #write_shaded_image(sdm2d, out='sdm2dz')
        #sdm2d = min_from_sdm3d(sdm3d, 1)
        #write_shaded_image(sdm2d, out='sdm2dx')
        #sdm2d = min_from_sdm3d(sdm3d, 2)
        #write_shaded_image(sdm2d, out='sdm2dy')
