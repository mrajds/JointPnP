...
from ll4ma_pick_n_place.segmentation_client import SegmentationClient, get_objgrid, \
    get_multi_objgrid, update_object_world_pose
...
#SOLUTION = np.array([1.8140774091487335, 1.1022285482463845, 2.1158387047859186, 1.8849516287748906, -2.442935184604368, -0.6125022785292408, 2.040380990126016, -0.26810471863125784, 0.813674105910367, 0.17035543680032775, -1.8849611283167376, 0.9569403884429961, -1.33093763926533, 1.1945850388543509, 0.3881740185452035])
...
def joint_init(pblm, num_inits=5, full_cost=False, debug=False):
    ...
    for ini in range(num_inits):
        ...
        if debug:
            ...
            #pyplot.show()
        inits.append(x0)
        costs.append(sigmoid(pblm.validate_grasp_collision(x0)) * \
        sigmoid(pblm.cost(x0, split=True)[1]))
    #print(inits)
    print(costs)
    return np.array(inits)[np.argsort(costs)]
...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        ...
        #place_heuristic = heurs.packing_heuristic(Env_List, obj_grid)
        #exit(0)
        pblm = PnP_Problem_CS(obj_grid, Env_List, genv_list=GEnv_List, G_Wt=g_wt, P_Wt=p_wt,\
                              place_heuristic = place_heuristic, \
                              grasp_mode=GRASP_MODE)

        eobj_grid, eclutter_grids, clutter_poses = get_multi_objgrid(True)
        eclutter_grids.append(eobj_grid)

        Env_List = {}
        for i, ze in enumerate(eclutter_grids):
            ze.gen_sd_map()
            ze.Trans = pblm.ptable_world_mat @ ze.compute_pretrans()
            Env_List[f'{i}_p'] = ze
            ze.get_mesh(f'{i}_p'+'.stl')
            planner.add_object_mesh(f'{i}_p', 'place_table_corner', ze.get_pose().pose)
        pblm.Env_List = Env_List

        update_object_world_pose(obj_grid.pose)
