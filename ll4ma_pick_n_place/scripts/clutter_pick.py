...
SOLUTION = np.array([-2.6649766898356724, -0.8799550241745538, -0.21899901452408269, 1.6809850365466223, 0.47126470768028567, 0.922385761228703, -1.616201562866802, -0.08931528650267714, 0.8530703017012479, 0.365246256563642, -1.8849501367545711, 1.5613905741248306, -1.8265527847638323, 1.1970530656047869, 0.33711044770998155])
...
def place_init(pblm):
    ...
    for i in range(free_pts.shape[0]):
        ...
        if free_pts[i][0] < place_heuristic.place_bounds[0][0] or \
           free_pts[i][1] < place_heuristic.place_bounds[0][1]:
            free_costs[i] = 1000
...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        ...
        for ninit,init in enumerate(inits[:]):
            ...
            if not input("continue?").lower() in ['y', 'yes']:
                continue
