    ...
    with tf.Session() as sess:
        ...
        while grasp_loader.epochs_completed < training_epochs:
            ...
            grasp_configs, grasp_voxel_grids, grasp_obj_sizes, grasp_labels = \
                    grasp_loader.next_batch(batch_size, as_dict=False)
        ...
        while grasp_loader.epochs_completed < testing_epochs:
            (grasp_configs, grasp_voxel_grids, grasp_obj_sizes, grasp_labels), \
                                grasp_top_labels = grasp_loader.next_batch(
                                                    batch_size, top_label=True, as_dict=False)
            # Don't need to feed the learning rate of voxel ae, 
            # since only the optimizer in prior_network is used.
            feed_dict = {prior_net.voxel_ae.is_train: (is_train and update_voxel_enc),
                        prior_net.voxel_ae.partial_voxel_grids: grasp_voxel_grids,
...
if __name__ == '__main__':
    ...
    prior_model_folder = '/models/reflex_grasp_inf_models/grasp_voxel_inf_prior/'
    ...
    if is_train:
        ...
        logs_path = '/home/mohanraj/tf_logs/prior_net_logs'

        if update_voxel_enc:
            if pretrain_voxel_enc:
                prior_model_path = pkg_path + prior_model_folder + 'prior_net_update_enc_pre.ckpt'
            else:
                prior_model_path = pkg_path + prior_model_folder  + 'prior_net_update_enc_scratch.ckpt'
        else:
            ...
            prior_model_path = pkg_path + prior_model_folder + 'prior_net_reflex.ckpt'

        voxel_ae_model_path = pkg_path + '/models/reflex_grasp_inf_models/voxel_ae/'
