...
from ll4ma_pick_n_place.segmentation_client import SegmentationClient, get_objgrid, \
    get_multi_objgrid
from ll4ma_pick_n_place.opt_problem import PnP_Problem_CS, Grasp_Collision, publish_env_cloud
...
#SOLUTION = np.array([-2.381054637763477, -1.4535592843739065, -2.6116086796151974, -0.6082431482826856, -0.221209551074933, -0.31653375056738114, 1.3794950579306984, -0.7198096005079067, 0.9615114179809966, 2.6703907682037973, 1.8849246480098614, 2.0663463525373866, -1.650555595918088, 2.186568532974365, 0.26221440688656117])
...
def grasp_init(pblm, num_inits=5):
    ...
    for ini in range(num_inits):
        ...
        costs.append(pblm.validate_grasp_collision(x0))
...
def place_init(pblm):
    ...
    for i in range(free_pts.shape[0]):
        ...
        if free_pts[i][0] < place_heuristic.place_bounds[0] or \
           free_pts[i][1] < place_heuristic.place_bounds[1]:
            free_costs[i] = 1000
...
def joint_init(pblm, num_inits=5, full_cost=False, debug=False):
    ...
    OI = pblm.get_2d_obj_grid()
    rospy.loginfo('Object generated')
    convr = ConvNet([1,1]+list(OI.shape))
    SI = pblm.flatten_3d_grid(SDG, bound=False)#pblm.get_2d_sd_grid()
    SI = convr.clean_2d(SI)
    SI[SI>0.5] = 1
    SI[SI<0.5] = 0
    COI = get_quad_convs(SI, OI, False)
    ...
    if debug:
        ...
        img = Image.fromarray(np.uint8(OI*255), 'L')
        img.save(fig_loc+f'obj.png')

        img = Image.fromarray(np.uint8(SI*255), 'L')
        img.save(fig_loc+f'env.png')
        #pyplot.savefig(f'obj.png')
        #pyplot.imshow(SI, cmap='gray')
        #pyplot.savefig(f'env.png')
        #for m in range(4):
            #pyplot.imshow(COI[m], cmap='gray')
            #pyplot.savefig(fig_loc+f'o_{m}.png')
    ...
    for ini in range(num_inits):
        ...
        gcoll_min = 1000
        min_ginit = pblm.x0 * 0
        for _ in range(100//num_inits):
            pblm.initialize_grasp(gidx)
            gcoll = pblm.validate_grasp_collision(pblm.x0)
            if gcoll < gcoll_min and not np.all(pblm.x0 == 0):
                gcoll_min = gcoll
                min_ginit = copy.deepcopy(pblm.x0)
        pblm.x0 = min_ginit
        if np.all(pblm.x0 == 0) or pblm.x0 is None:
            rospy.loginfo('No feasilbe grasps found skipping')
            continue
        RI, min_z, max_z = pblm.get_2d_gripper_kernel()
        convr = ConvNet([1,1]+list(RI.shape))
        ...
        CI[:, :int(place_heuristic.place_bounds[0][0]*1e2), :] = 100
        CI[:, int(place_heuristic.place_bounds[1][0]*1e2):, :] = 100
        CI[:, :, :int(place_heuristic.place_bounds[0][1]*1e2)] = 100
        CI[:, :, int(place_heuristic.place_bounds[1][1]*1e2):] = 100
        ...
        if n_c == 0:
            rospy.loginfo('No placement locations found')
            continue
        #free_pts = full_free_pts
        #free_pts = KMeans(n_clusters=n_c, n_init="auto").fit(full_free_pts).cluster_centers_
        free_pts = full_free_pts
        free_costs = np.zeros(free_pts.shape[0])
        for i in range(free_pts.shape[0]):
            free_costs[i] = place_heuristic.cost(free_pts[i])
            if free_pts[i][0] < place_heuristic.place_bounds[0][0] or \
               free_pts[i][0] > place_heuristic.place_bounds[1][0] or \
               free_pts[i][1] < place_heuristic.place_bounds[0][1] or \
               free_pts[i][1] > place_heuristic.place_bounds[1][1]:
                free_costs[i] = np.inf
        ranked_pts = free_pts[np.argsort(free_costs)]
        #ranked_pts = ranked_pts * 1e-3
        ...
        if debug:
            #pyplot.imshow(RI[0][0])
            #pyplot.show()
            #pyplot.imshow(SIr, cmap='gray')
            #pyplot.savefig(fig_loc+f'{ini}_er.png')
            #pyplot.imshow(RI, cmap='gray')
            #pyplot.savefig(fig_loc+f'{ini}_r.png')
            img = Image.fromarray(np.uint8(RI*255), 'L')
            img.save(fig_loc+f'{ini}_r.png')

            VizI = np.zeros(CI.shape)
            VizI[CI<=1e-3] = free_costs[:]
    
            for m in range(4):
                #pyplot.imshow(CRI[m], cmap='gray')
                #pyplot.savefig(fig_loc+f'r{ini}_{m}.png')
                #pyplot.imshow(CI[m], cmap='gray')
                pyplot.imshow(VizI[m])
                pyplot.show()
                img = Image.fromarray(np.uint8(CI[m]*255), 'L')
                img.save(fig_loc+f'{ini}_{m}.png')
            #pyplot.show()
        inits.append(x0)
...
if __name__ == '__main__':
    ...
    def shutdown():
        ...
        planner.remove_objects()
        os.remove(GRASP_OBJ_FN+'.stl')
        for n in planner.coll_obj_names:
            os.remove(n+'.stl')
    ...
    while not rospy.is_shutdown():
        ...
        obj_grid, clutter_grids, clutter_poses = get_multi_objgrid(True)
        GEnv_List = {}
        #obj_grid.gen_sd_map()

        for i, grid in enumerate(clutter_grids):
            grid.gen_sd_map()
            grid.set_pose(clutter_poses[i],sref=False)
            grid.get_mesh(f'{i}'+'.stl')
            GEnv_List[f'{i}'] = grid

        publish_env_cloud(GEnv_List, frame_id='object_pose')

        start = time.time()
        
        g_wt, p_wt = get_wts()
        #place_heuristic = define_place_heuristic()
        place_heuristic = heurs.y_inline_heuristic(0.60)
        #exit(0)
        pblm = PnP_Problem_CS(obj_grid, Env_List, genv_list=GEnv_List, G_Wt=g_wt, P_Wt=p_wt,\
                              place_heuristic = place_heuristic, \
        ...
        #obj_grid.get_mesh()
        planner.add_object_bb([0.1, 0.1, 0.25])

        if input('add_clutter_mesh?').lower() == 'y':
            for key in GEnv_List:
                pose_ = GEnv_List[key].get_pose().pose
                #planner.add_object_mesh(key, pose=pose_)
                planner.add_object_bb([0.1, 0.1, 0.25], key, pose_)
        rospy.loginfo("mesh added")
        #planner.add_object_bb(np.array(obj_grid.true_size[[1,0,2]]) + [0.1,0.1,0.0])
        #input('check memory')
        ...
        for ninit,init in enumerate(inits[:]):
            ...
            pre_grasp_joints = pblm.get_approach_head(solret.solution[pblm.G_IDX])
            #pre_grasp_joints = solret.solution[pblm.G_IDX]
        ...
        print(pblm.object_pose_from_joints(solret.solution))
        ...
        if input('Add env?').lower() in ['y', 'yes']:
            obj_n = None
            while (not obj_n) or (obj_n in Env_List): 
                obj_n = input('Input object identifier: ')
            Env_List[obj_n] = obj_grid
            planner.add_object_bb([0.1, 0.1, 0.25], obj_n, obj_grid.get_pose().pose, 'place_table_corner')
