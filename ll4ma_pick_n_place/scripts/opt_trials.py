...
from ll4ma_pick_n_place.data_utils import get_object_name, save_problem, restore_problem
from ll4ma_pick_n_place.visualizations import *
...
def parse_args():
    ...
    parser.add_argument('--pick', action='store_true', dest='pick')
    parser.add_argument('--place', action='store_true', dest='place')
...
if __name__ == '__main__':
    rospy.init_node('pnp_solver_node')
    old_Env_List = None
    session_mode = False
    args = parse_args()
    if args.session is None:
        rospy.loginfo("Starting a new session")
    else:
        pkl_f = args.session
    ...
    resume = args.resume and session_mode
    redo = not args.resume and session_mode
    ...
    rospy.loginfo(f"Solving for {obj_n}")
    ...
    if not session_mode or resume:
        spawn_model_client(obj_n)
        obj_grid = get_objgrid()
        pblm = PnP_Problem_CS(obj_grid, G_Wt=args.G_Wt, P_Wt=args.P_Wt)

    if redo or args.pick or args.place:
        obj_grid = old_obj_grid
        pblm = PnP_Problem_CS(old_obj_grid, G_Wt=args.G_Wt, P_Wt=args.P_Wt)
        pblm.obj_world_mat = old_obj_trans
        #old_x0[:] = np.array([ 1.33763664,-2.05420948, 0.55636431,
        #                       -1.37665203, -0.66050691,
        #                       -2.0943951, 0.59049175, -1.62512306,
        #                       1.8688708 , -1.75444614, 1.56508221,
        #                       -0.49854536, -0.92949744, 2.7573337 ,
        #                       0.41342231]).reshape(15,1)
        pblm.x0 = old_x0
        if args.place:
            print(pblm.x0.shape)
            print(solret.solution.shape)
    ...
    solver0 = NewtonMethodWolfeLine(pblm)
    #solver2 = BFGSMethod(pblm) #, FP_PRECISION=1e-12)
    solver2 = PenaltyMethod(pblm, "BFGS") #, FP_PRECISION=1e-12)#GradientDescent(pblm)
    solver2 = AugmentedLagranMethod(pblm, "BFGS") #, FP_PRECISION=1e-12)#GradientDescent(pblm)

    start = time.time()
    solret = solver2.optimize(pblm.x0, max_iterations=max_iters)#, update_cond=lambda x: x>0)
    ...
    obj_grid.gen_sd_map()
    pblm.Env_List[pblm.new_id()] = obj_grid
    pblm.publish_env_cloud()

    save_problem(pblm, solret)
    
    hand.set_preshape(solret.solution[pblm.PRESHAPE])
    plan_traj = planner.plan_to_joint(solret.solution[pblm.G_IDX])
